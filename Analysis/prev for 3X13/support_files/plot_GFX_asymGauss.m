...
job.plot_walkSpeedComparison =0; % focuses on the gaussian params comparing 2 walk speeds
...
job.plot_PropSame_perPcnt_perAlign=1;
...
if job.plot_walkSpeedComparison==1
    ...
    load('GFX_asymGaussData.mat');
    subplot(132); cla
    useCols= {'k', 'b', 'r'};
    % include group data for comparison?
    [nsubs,nconds, nsamps]= size(GFX_asymgauss_Cond_x_Yfit);
    %

    for icond= 2:3%:3 % first cond is 'all'
        plotme = squeeze(GFX_asymgauss_Cond_x_Yfit(:,icond,:));
    ...
    usesubspots= [3,6,9];%
    titlesAre = {'Mean (mu)', 'width (LHS)', 'width (RHS)'};
    ylimsat= [-20 20; 0 500 ;0 500 ];
    tat = [17, 450,450]; % text height

    for iparam=1:3 % mean and std of gaussian:

        paramData = squeeze(GFX_asymgauss_Cond_xparam(:, :,iparam)); % all 3 walk speeds:
        subplot(3,3, usesubspots(iparam));
        ...
        text(0, tat(iparam), txtmsg, 'HorizontalAlignment','left');
...
if job.plot_walkQuantileComparison==1
    ...
    for iAlign= 1:length(alignFlds)
        ...
        for icond= 1:3%:3 % first cond is 'all', slow , fast.
            ...
            cla
            % GFX_gauss_Cond_x_Gait_x_Yfit
            plotme = squeeze(GFX_asymgauss_Cond_x_Gait_x_Yfit(:,icond,iAlign,:,:));
        ...
        usesubspots= [3,6,9];
        titlesAre = {'Mean (mu)', 'gauss width(LHS)', 'gauss width (RHS)'};
        ylimsat= [-20 20; 0 500; 0 500 ];
        ...
        %%
        for iparam=1:3
            paramData = squeeze(GFX_asymgauss_Cond_x_Gait_param(:, :, iAlign,:,iparam)); % speeds: all, slow, fast
        ...
        %%
        writetable(jtable,['Gauss_asymGaitParams_' alignmentWas{iAlign}   '.csv'])
    end
...
if job.plot_PropSame_perPcnt_perAlign==1;
    ...
    for iAlign= 5%1:length(alignFlds);
 figure(2);
    clf
        hold on
        % all speeds:
...
%% mea over SOAs?
    figure(3); clf
    for iplot=1:3
  tmp= squeeze(plotData{iplot});
  plt = squeeze(nanmean(tmp,2));
  pM = squeeze(nanmean(plt,1));
  stE = CousineauSEM(plt);
  shadedErrorBar(1:length(pM), pM, stE,{'color', spdCols{iplot}},1); hold on;
  hold on;
%   title('Avarege over all SOAs')
    end
